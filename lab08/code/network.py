...
class Network:
    ...
    def backward(self):
        ...
        # if self.layers[-1].output.ndimension() == 0:
        #     print('RESHAPING')
        #     # TODO: GO INTO LAYERS AND DO THIS WITH THE REASSIGNING
        #     self.layers[-1].output = self.layers[-1].output.reshape(1)
        #     print(f'network output shape = {self.layers[-1].output.shape}')
        
        self.layers[-1].set_grad(torch.ones(self.layers[-1].output.shape, dtype=torch.float32))
